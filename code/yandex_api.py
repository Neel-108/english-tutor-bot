"""
yandex_api.py
YandexGPT Pro 5.1 API integration with rate limiting and retry logic
"""
...
# Global rate limiting - prevents API overload
_last_request_time = 0.0


async def call_yandex_gpt(prompt: str) -> dict:
    """
    Call YandexGPT Pro 5.1 with rate limiting and retry logic
            ...
            async with aiohttp.ClientSession(
                ...
                async with session.post(
                    ...
                    if response.status == 429:
                        wait_time = 2 ** submit_attempt  # Exponential backoff: 1s, 2s, 4s
    ...
    max_poll_attempts = 40  # 40 attempts over ~120 seconds
    base_delay = 2
        ...
        try:
            async with aiohttp.ClientSession(
                ...
                async with session.get(result_url, headers=headers) as response:
                    ...
                    if poll_attempt < 5:
                        delay = base_delay * (1.5 ** poll_attempt)  # 2s, 3s, 4.5s, 6.75s, 10s
                    else:
                        delay = 3  # Constant 3s after initial backoff
            ...
            if poll_attempt == max_poll_attempts - 1:
                raise Exception("YandexGPT operation timeout - no response after 120 seconds")
    ...
    raise Exception("YandexGPT operation timeout - exceeded maximum polling attempts")
